...
namespace JiaHang.Projects.Admin.BLL.Relation
{
    ...
    public class CurrentUserRouteBLL
    {
        ...
        public CurrentUserRouteModel GetRoutes(string currentUserId, bool isAdmin = false)
        {
            //根据当前用户id 获取所有用户组
            ...
            if (!isAdmin)
            {
                var query = (from a in _context.SysUserInfo
                             ...
                             join b in _context.SysUserGroupRelation
                             on a.UserId equals b.UserId
                             join c in _context.SysUserGroup
                             on b.UserGroupId equals c.UserGroupId
                             select c).ToList();
                var groups = new List<SysUserGroup>();
                var group_all_data = _context.SysUserGroup.ToList();
                foreach (var obj in query)
                {
                    groups.AddRange(RecursiveGetUserGroup(group_all_data, obj));
                }

              var  modules_query = (from a in _context.SysModule
                                 join b in _context.SysModuleUserRelation
                                 on a.Id equals b.ModuleId
                                 join c in groups on b.UserGroupId equals c.UserGroupId
                                 select a).ToList();
                var module_user = from a in _context.SysModule
                                  join b in _context.SysModuleUserRelation
                                  on a.Id equals b.ModuleId
                                  where b.UserId == currentUserId
                                  select a;
                modules_query.AddRange(module_user.ToList());
                var module_all_data = _context.SysModule.ToList();
            ...
            var routes = (from a in _context.SysControllerRoute
                          ...
                          join d in _context.SysMethodRoute on a.SysControllerRouteId equals d.ControllerId
                          //join e in _context.SysMethodRoute
                          //on a.AreaId equals e.Id
                          //into e_temp from e_ifnull in e_temp.DefaultIfEmpty()
                          orderby c.SortValue ,a.SortValue descending
